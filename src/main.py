...
def make_hparams():
    return nkutil.HParams(
        ...
        special_label_re=r"EDITED",
        special_label_weight=2.0,
...
def run_train(args, hparams):
    ...
    print("Loaded {:,} development examples.".format(len(dev_treebank)), flush=True)
    ...
    def check_dev():
        ...
        print(
            " dev-Efscore: {}".format(dev_efscore),
            flush=True
        )
...
def run_test(args):
    print("Loading test trees from {}...".format(args.test_path), flush=True)
    test_treebank = trees.load_trees(args.test_path)
    print("Loaded {:,} test examples.".format(len(test_treebank)), flush=True)

    print("Loading model from {}...".format(args.model_path_base), flush=True)
    ...
    print("Parsing test sentences...", flush=True)
    start_time = time.time()
    ...
    if args.test_path_raw is not None:
        print("Comparing with raw trees from", args.test_path_raw, flush=True)
    ...
    test_efscore = evaluate_EDITED.Evaluate(test_treebank, test_predicted)

    print(
        "test-Efscore:\n{}".format(test_efscore.table()),
        flush=True
    )
...
def run_ensemble(args):
    ...
    for model_path_base in args.model_path_base:
        print("Loading model from {}...".format(model_path_base), flush=True)
    ...
    print(
        "test-Efscore {}".format(test_efscore.table()),
        flush=True
    )
...
def run_parse(args):
    if args.output_path != '-' and os.path.exists(args.output_path):
        print("Error: output file already exists:", args.output_path, flush=True)
        return
    ...
    print("Parsing sentences...", flush=True)
    ...
    for start_index in range(0, len(sentences), args.eval_batch_size):
        ...
        if args.output_path == '-':
            for p in predicted:
                print(p.convert().linearize(), flush=True)
        else:
    ...
    if args.output_path != '-':
        ...
        print("Output written to:", args.output_path, flush=True)

#%%
def run_viz(args):
    assert args.model_path_base.endswith(".pt"), "Only pytorch savefiles supported"

    print("Loading test trees from {}...".format(args.viz_path), flush=True)
    viz_treebank = trees.load_trees(args.viz_path)
    print("Loaded {:,} test examples.".format(len(viz_treebank)), flush=True)
    ...
    print("Parsing viz sentences...", flush=True)
...
def main():
    ...
    subparser.add_argument("--train-path", default="swbd-data/autopos-punct-pw/train.txt")
    subparser.add_argument("--dev-path", default="swbd-data/autopos-punct-pw/dev.txt")
    subparser.add_argument("--results-path", default=None)
    subparser.add_argument("--epoch1-hurdle", default=0.5, type=float,
                           help="Stop training if epoch 1 efscore less than this value")
    subparser.add_argument("--epoch10-hurdle", default=0.75, type=float,
                           help="Stop training if epoch 10 efscore less than this value")
    ...
    subparser.add_argument("--print-minibatches", action="store_true")
    ...
    subparser.add_argument("--test-path", default="swbd-data/autopos-nopunct-nopw/test.txt")
    ...
    subparser.add_argument("--test-path", default="swbd-data/autopos-nopunct-nopw/dev.txt")
    subparser.add_argument("--eval-batch-size", type=int, default=100)
